...
class MainWatcher:
    ...
    def connect(self):
        while not self.connected and self.retry:
            try:
                ...
                self.connection: amqpstorm.Connection = amqpstorm.Connection(
                ...
                self.channel.exchange.declare(
                    exchange="homedaemon", exchange_type="topic", auto_delete=False
                )
                ...
                self.channel.basic.consume(
                    queue="main_queue", callback=self.on_message, no_ack=True
                )
                self.connected = True
    ...
    def register_home(self, homeid: str):
        ...
        self.homes[homeid] = HomeManager(
            self.channel, homeid, self.db_connection.get_db(homeid)
        )
...
class HomeManager(Thread):
    ...
    def start(self):
        ...
        self.channel.queue.bind(
            queue=self.homeid,
            exchange="homedaemon",
            routing_key=f"homedaemon.{self.homeid}.reports",
        )
        self.channel.basic.consume(
            callback=self.on_event, queue=self.homeid, no_ack=True
        )
    ...
    def __del__(self):
        self.channel.queue.unbind(
            self.homeid, "homedaemon", routing_key=f"homedaemon.{self.homeid}.reports"
        )
...
if __name__ == "__main__":
    ...
    config.load_config_from_file(
        os.environ.get("CONF_FILE", "/etc/homedaemon/homed.json")
    )
